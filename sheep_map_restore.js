(() => {
  ...
  function 构建星图外壳节点面板(选项 = {}) {
    ...
    if (!isSubMapView || isFreeSelection) return null;

    const focusItem = getItemByName(指定焦点名)
  ...
  function buildPreviewPayloadFromRawLocation(nodeName, rawLocation, container = mapState.snapshot, parentMapId = '', 已访问节点集合 = new Set()) {
    ...
    const 下一层已访问节点集合 = new Set(已访问节点集合);
    下一层已访问节点集合.add(安全节点名);
    ...
    childEntries.forEach(([childName, childValue]) => {
      ...
      if (hasChildren || hasDynamicChildren) {
        const nestedPayload = buildPreviewPayloadFromRawLocation(childName, child, container, currentMapId, 下一层已访问节点集合);
        if (nestedPayload) {
    ...
    dynamicEntries.forEach(([动态地点名, 动态地点数据]) => {
      ...
      const 子预览载荷 = buildPreviewPayloadFromRawLocation(动态地点名, 动态节点, container, currentMapId, 下一层已访问节点集合);
      if (子预览载荷) {
        previewChildMaps[动态地点名] = 子预览载荷;
        availableChildMaps[动态地点名] = 子预览载荷.current_map_id;
      }
  ...
  function canEnterPreviewNode(nodeName = mapState.selectedNode, snapshot = mapState.snapshot) {
    ...
    const currentMapId = toText(snapshot.currentMapId, '');
    const currentFocusName = toText(snapshot.currentFocusName || snapshot.currentLoc, '');
    const localNodeName = String(nodeName).split('-').pop();
    if (currentMapId && currentMapId !== 'map_douluo_world' && localNodeName === currentFocusName) {
      return false;
    }
  ...
  function 获取地图标签边缘测量签名(画布, 结构签名 = '') {
    ...
    return [
      ...
      mapState.selectedNode || '',
      mapState.currentNode || '',
      mapState.selectedFreePoint ? `${mapState.selectedFreePoint.x},${mapState.selectedFreePoint.y}` : '',
      mapState.currentFreePoint ? `${mapState.currentFreePoint.x},${mapState.currentFreePoint.y}` : '',
      Number(getMapRenderZoom()).toFixed(3),
      Math.round(toNumber(mapState.panX, 0)),
      Math.round(toNumber(mapState.panY, 0)),
  ...
  function isSelectionCurrentBranch() {
    ...
    if (判断地图节点为当前位置(mapState.selectedNode)) return true;
  ...
  function getMapTravelPreview() {
    ...
    if (!mapState.selectedFreePoint && 判断地图节点为当前位置(mapState.selectedNode)) return null;
  ...
  function hasPendingTravelRequestForTarget() {
    ...
    if (mapState.pendingTravelRequest.target_loc !== '无' && 判断地图节点为当前位置(mapState.pendingTravelRequest.target_loc)) return false;
  ...
  function queueMapTravelRequest() {
    ...
    if (request.target_loc !== '无' && 判断地图节点为当前位置(request.target_loc)) {
      mapState.pendingTravelRequest = null;
      return null;
    }
  ...
  function commitMapTravel(options = {}) {
    ...
    if (mapState.baseSnapshot && mapState.baseSnapshot.activeChar) {
      try {
        ...
        const 启程句 = `${移动动作}前往【${移动显示目标}】${targetTerrainLine ? ` · 地形：${targetTerrainBrief}` : ''}`;
        ...
        const logMsg = `[系统仲裁] 玩家${移动动作}，经过 ${request.est_duration} 的跋涉，现已抵达新地点：【${移动显示目标}】。${targetTerrainLine ? `\n${targetTerrainLine}` : ''}`;
...
请结合当前设施、在场角色与地点功能，自然写出这次行动的过程、收获与后续推进；若当前节点并不适合该动作，也请在剧情里明确说明阻碍原因。正文不要直接复述 JSONPatch 或系统术语。`;
  ...
  function performMapAction(actionType) {
    ...
    if (原始动作 === 'travel') {
      ...
      if (mapState.selectedFreePoint) {
        if (!hasPendingTravelRequestForTarget()) queueMapTravelRequest();
        commitMapTravel();
        return true;
      }
  ...
  function describeTravelToNode(nodeName) {
    return withSelectedNodeSnapshot(nodeName, targetItem => {
      if (判断地图节点为当前位置(targetItem.name)) {
  ...
  function renderMapInfoState() {
    ...
    if (mapState.pendingTravelRequest && mapState.pendingTravelRequest.target_loc !== '无' && 判断地图节点为当前位置(mapState.pendingTravelRequest.target_loc, snapshot)) {
      mapState.pendingTravelRequest = null;
    }
    ...
    const resolveIndexedCharacterEntries = (locationName, rawLocationName = '') => {
      ...
      const 原始路径片段 = 拆分地点路径(rawLocationName);
      const 显示路径片段 = 拆分地点路径(locationName);
      const locationCandidates = [
        toText(rawLocationName, ''),
        归一地点名(rawLocationName),
    ...
    const 可交互人物名列表 = characterEntries.filter(entry => entry && entry.可交互 === true).map(entry => entry.name);
    let selectedNpc = toText(mapState.selectedNpc, '');
    if (selectedNpc && !可交互人物名列表.includes(selectedNpc)) selectedNpc = '';
    if (!selectedNpc && 可交互人物名列表.length === 1) selectedNpc = 可交互人物名列表[0];
    ...
    const npcBrowserActions = getNpcActionCandidates(panelMode === 'selection' ? focusItem : detailPanelItem, 可交互人物名列表.length);
      ...
      : (inPreview && !previewCurrentBranch)
        ...
        : (!focusItem || !characterEntries.length)
          ...
          : `<div class="map-npc-roster-head"><span>${escapeMapHtml(在场人物节点文本)}</span><span>${escapeMapHtml(selectedNpc ? `已选 ${selectedNpc}` : `全部 ${characterEntries.length} 人`)}</span></div>${characterEntries.map(entry => {
            ...
            const 人物名称HTML = 可交互人物
              ? `<button type="button" class="map-npc-name${selectedClass}" data-map-npc-select="${escapeMapHtml(entry.name)}" title="选中 ${escapeMapHtml(entry.name)}">${escapeMapHtml(entry.name)}</button>`
              : `<span class="map-npc-name">${escapeMapHtml(entry.name)}</span>`;
            return `<div class="map-npc-card${selectedClass}"><div class="map-npc-card-head">${人物名称HTML}<span class="map-event-chip${selectedClass ? ' live' : ''}">${selectedClass ? '已选' : (可交互人物 ? '在场' : '子节点')}</span></div>${metaHtml}${actionHtml}</div>`;
          }).join('')}`;
