(() => {
  ...
  function 构建星图外壳焦点HTML(snapshot, focusItem, 选项 = {}) {
    ...
    const 状态文本 = focusItem ? toText(focusItem.state, '暂无') : (选项.自由坐标 ? '已选中' : '暂无');
    const 功能文本 = focusItem ? (formatBehaviorLabels(focusItem.interactions, getNodeInteractionLabel) || '无') : '查看坐标 / 开始移动';
    const 可用文本 = focusItem
      ? (canEnterPreviewNode(focusName, snapshot)
        ? '可进入子图'
        : (Array.isArray(focusItem.services) && focusItem.services.length ? formatBehaviorLabels(focusItem.services, getNodeServiceLabel) : '无'))
      : '无固定设施';
    const 掌控文本 = focusItem ? toText(focusItem.faction, '未知') : '未知';
