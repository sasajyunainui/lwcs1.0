(() => {
  ...
  async function 执行地图维护操作(操作 = '') {
    ...
    const 当前地点节点 = 动态地点 || 获取地点节点及父级(焦点名, mapState.baseSnapshot || mapState.snapshot)?.节点 || {};
    const 原现状描述 = toText(当前地点节点.现状描述, '').trim();
    const 维护描述 = `${原现状描述 ? `${原现状描述}\n` : ''}[地图维护] ${当前操作 === 'damage' ? '破坏' : '修复'}：${描述值 || (当前操作 === 'damage' ? '地点受损，服务暂停。' : '地点修复，服务恢复。')}`;
    const patchOps = [
      { op: 'add', path: 状态路径, value: 状态值 },
      { op: 'add', path: `${基础路径}/现状描述`, value: 维护描述 },
...
本次行动收益与系统播报已由前端预结算写回，行动耗时仅用于本轮时间裁定。请结合当前设施、在场角色与地点功能，自然写出这次行动的过程、收获与后续推进；若当前节点并不适合该动作，也请在剧情里明确说明阻碍原因。正文不要输出变量维护指令或系统术语。`;
  ...
  function applyMapWorldTransform(options = {}) {
    ...
    worldEls.forEach(world => {
      ...
      if (!关联画布) return;
      setMapNodeStyle(world, 'transform', 变换);
