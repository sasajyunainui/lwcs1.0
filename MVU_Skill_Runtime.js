...
function 读取副职业核心技艺文本_V1(_副职业名 = '', 等级 = 0) {
  ...
  if (等级数值 >= 9) return '天锻（四字斗铠/红级以上机甲设计/制造/修理）';
  if (等级数值 >= 7) return '魂锻（三字斗铠/红级机甲设计/制造/修理）';
  if (等级数值 >= 5) return '灵锻（二字斗铠/黑级机甲设计/制造/修理）';
  if (等级数值 >= 3) return '千锻（一字斗铠/紫级机甲设计/制造/修理）';
  if (等级数值 >= 1) return '百锻（黄级机甲设计/制造/修理）';
  return '未掌握';
}
...
function getBaseStats(lv) {
  ...
  let 精神力上限 = 等级值;
  if (等级值 <= 20) 精神力上限 = 等级值;
  else if (等级值 <= 30) 精神力上限 = 20 + ((80 - 20) / 10) * (等级值 - 20);
  else if (等级值 <= 40) 精神力上限 = 80 + ((420 - 80) / 10) * (等级值 - 30);
  else if (等级值 <= 45) 精神力上限 = 420 + ((600 - 420) / 5) * (等级值 - 40);
  else if (等级值 <= 50) 精神力上限 = 600 + ((800 - 600) / 5) * (等级值 - 45);
  else if (等级值 <= 60) 精神力上限 = 800 + ((1300 - 800) / 10) * (等级值 - 50);
  else if (等级值 <= 70) 精神力上限 = 1300 + ((1800 - 1300) / 10) * (等级值 - 60);
  else if (等级值 <= 80) 精神力上限 = 1800 + ((2200 - 1800) / 10) * (等级值 - 70);
  else if (等级值 <= 90) 精神力上限 = 2200 + ((2450 - 2200) / 10) * (等级值 - 80);
  else if (等级值 < 98) 精神力上限 = 2450 + ((2600 - 2450) / 8) * (等级值 - 90);
  else if (等级值 === 98) 精神力上限 = 2600;
  else if (等级值 === 99) 精神力上限 = 3000;
  else if (等级值 === 99.5) 精神力上限 = 4000;
...
var SKILL_MECHANISM_NAME_TO_PROTOTYPES_V1 = Object.freeze({
  ...
  状态交换: [原型编译条目('状态交换')],
  状态转移: [原型编译条目('状态转移')],
  '强制绑定/锁定': [原型编译条目('状态施加', { 状态: '位移限制', 数值: '-15%', 副数值: '-10%' })],
...
var 技能状态选项_V1 = Object.freeze(['中毒', '流血', '灼烧', '冻伤', '持续创伤', '持续恢复', '迟缓', '资源燃烧', '眩晕', '沉默', '致盲', '禁疗', '治疗反转', '隐匿', '探查屏蔽', '共享视野', '护盾', '无视异常', '霸体', '标记', '封技', '位移限制', '护卫', '嘲讽', '防御剥夺', '精神抗性剥夺', '虚弱', '失控', '反噬', '精神紊乱', '魂力枯竭', '僵直', '麻痹', '混乱']);
var 技能负面状态选项_V1 = Object.freeze(['中毒', '流血', '灼烧', '冻伤', '持续创伤', '迟缓', '资源燃烧', '眩晕', '沉默', '致盲', '禁疗', '治疗反转', '标记', '封技', '位移限制', '嘲讽', '防御剥夺', '精神抗性剥夺', '虚弱', '失控', '反噬', '精神紊乱', '魂力枯竭', '僵直', '麻痹', '混乱']);
...
var SKILL_PROTOTYPE_FIELD_OPTIONS_V1 = Object.freeze({
  ...
  伤害类型: Object.freeze(['物理伤害', '能量伤害', '精神伤害', '真实伤害']),
  ...
  条件类型: Object.freeze(['当前行动', '命中', '被闪避', '状态存在', '护盾', '生命比例', '体力比例', '魂力比例', '精神力比例', '生命数值', '体力数值', '魂力数值', '精神力数值', '目标', '使用者', '环境满足', '时间', '装备状态', '自身状态', '连携前提']),
...
var SKILL_UNIT_COST_TABLE_V1 = Object.freeze({
  伤害结算: Object.freeze({
    ...
    伤害类型: Object.freeze({ 物理伤害: 1, 能量伤害: 1.08, 精神伤害: 1.15, 真实伤害: 1.45 }),
  ...
  状态施加: Object.freeze({
    ...
    位移限制: Object.freeze({ 数值: 1.2, 副数值: 0.9, 固定COST: 20 }),
...
var 状态施加状态分类矩阵_V1 = Object.freeze({
  ...
  硬控封禁类: Object.freeze(['眩晕', '沉默', '致盲', '封技', '位移限制', '麻痹', '混乱', '僵直', '失控', '精神紊乱']),
...
function 机制是合法生成主机制_V1(机制名 = '', context = {}) {
  ...
  if (['辅助系', '治疗系', '食物系'].includes(系别) && 魂环位 <= 2 && ['直接伤害', '多段伤害', '持续伤害', '硬控', '节奏打断', '位移限制', '多属性削弱', '掌控压制', '掌控提升', '元素封禁', '禁疗'].includes(机制)) return false;
  return (
...
var 技能条件分支类型候选_V1 = Object.freeze(['当前行动', '命中', '被闪避', '状态存在', '护盾', '生命比例', '体力比例', '魂力比例', '精神力比例', '生命数值', '体力数值', '魂力数值', '精神力数值', '目标', '使用者', '环境满足', '时间', '装备状态', '自身状态', '连携前提', '单位文本']);
...
function 状态施加允许数值字段_V1(状态 = '') {
  ...
  return ['持续伤害类', '恢复资源类', '防御数值类'].includes(读取状态施加分类_V1(状态)) ||
    ['禁疗', '治疗反转', '标记', '嘲讽', '防御剥夺', '精神抗性剥夺', '虚弱', '迟缓', '资源燃烧', '位移限制', '失控', '反噬', '精神紊乱', '僵直', '麻痹', '混乱', '魂力枯竭'].includes(状态名);
}

function 状态施加允许副数值字段_V1(状态 = '') {
  return ['中毒', '流血', '灼烧', '冻伤', '持续创伤', '资源燃烧', '迟缓', '位移限制', '失控', '精神紊乱', '魂力枯竭', '僵直', '麻痹', '混乱'].includes(String(状态 || '').trim());
}
...
function 构建机制编译输入覆盖列表_V1(机制名 = '', 原型列表 = [], 上下文 = {}) {
  ...
  const 默认源 = 原型条目 => {
    ...
    if (原型 === '伤害结算') {
      ...
      字段.伤害类型 ??= String(伤害意图.伤害类型 || (系别 === '精神系' ? '精神伤害' : 系别 === '元素系' ? '能量伤害' : '物理伤害')).trim() || '物理伤害';
...
function 补足机制模板原型数值_V1(原型 = '', 字段 = {}, source = {}, 上下文 = {}) {
  ...
  if (原型 === '伤害结算') {
    ...
    if (字段.伤害类型 === undefined) 字段.伤害类型 = String(source.伤害类型 || source.damageType || '物理伤害').trim() || '物理伤害';
...
function 清理执行效果原型语义_V1(原型 = '', 字段 = {}, 上下文 = {}) {
  ...
  if (原型 === '伤害结算') {
    ...
    const 伤害类型 = String(字段.伤害类型 || '物理伤害').trim();
    字段.伤害类型 = SKILL_PROTOTYPE_FIELD_OPTIONS_V1.伤害类型.includes(伤害类型) ? 伤害类型 : '物理伤害';
    if (字段.伤害类型 === '真实伤害') {
      字段.结算标签 = '真实伤害';
      字段.抗性类型 = '真实无视';
    } else {
...
function 断言技能执行效果原型契约_V1(effectArray = [], path = '_效果数组', 上下文 = {}) {
  ...
  const 标记消费存在 = 技能执行效果数组存在匹配_V1(effectArray, effect => {
    const 原型 = String(effect?.原型 || '').trim();
    return (
      (原型 === '状态施加' && ['标记', '位移限制'].includes(String(effect?.状态 || '').trim())) ||
...
function buildSkillCombatProfile(blueprint, qualityCtx = {}) {
  ...
  const getDamageType = () => {
    if (type === '精神系') return '精神伤害';
    if (type === '食物系' || type === '控制系' || type === '元素系') return '能量伤害';
    return '物理伤害';
  };
  ...
  if (main === '伤害类') {
    ...
    战斗.技能分类 = '控制';
    效果意图.状态 = archetype;
    效果意图.持续回合 = ['硬控', '位移限制'].includes(archetype) ? 1 : 2;
    if (archetype === '软控') {
...
function 遍历直接结算预算效果_V1(效果数组 = [], 回调 = () => {}) {
  (Array.isArray(效果数组) ? 效果数组 : []).forEach(效果 => {
    if (!效果 || typeof 效果 !== 'object' || Array.isArray(效果)) return;
    回调(效果);
    技能执行嵌套效果数组字段表_V1.forEach(字段名 => {
      遍历直接结算预算效果_V1(效果?.[字段名], 回调);
    });
    (Array.isArray(效果.条件分支) ? 效果.条件分支 : []).forEach(分支 => {
      遍历直接结算预算效果_V1(分支?.替换效果, 回调);
      遍历直接结算预算效果_V1(分支?.追加效果, 回调);
    });
  });
}
...
function 计算直接结算收益预算_V1(技能 = {}, 上下文 = {}) {
  ...
  let 中绑定压力 = 0;
  const 压力来源列表 = [];
  遍历直接结算预算效果_V1(技能对象._效果数组 || [], 效果 => {
    const 压力 = 计算直接结算收益预算效果压力_V1(效果);
    const 高绑定 = Number(压力.高绑定压力 || 0);
    const 中绑定 = Number(压力.中绑定压力 || 0);
    高绑定压力 += 高绑定;
    中绑定压力 += 中绑定;
  ...
  if (
    ...
    return {
      ...
      前摇承载: 1,
      综合承载: 1,
      允许压力: Number((直接结算收益预算系数_V1.基准收益 * 直接结算收益预算系数_V1.宽容系数).toFixed(4)),
      预算系数: 1,
      通过: true,
      主要压力来源: 压力来源列表
...
var 技能状态施加转译语义_V1 = Object.freeze({
  ...
  霸体: 目标 => `使${目标}进入霸体状态`,
  封技: 目标 => `使${目标}进入封技状态`,
  位移限制: (目标, 效果) => `限制${目标}位移，${转译状态施加数值说明_V1('位移限制', 效果)}`,
...
function buildSingleSkillEffectSummary(effect) {
  ...
  switch (机甲) {
    ...
    case '位移限制':
      return '让' + target + '出手更别扭，位移幅度变小，闪避和反应也会下降' + 持续文本;
    case '单属性削弱':
    case '多属性削弱':
...
var NUMERIC_STAT_BONUS_KEYS = ['力量', '防御', '敏捷', '体力上限', '精神力上限'];
...
var GoldenDragonSkills = {
  1: {
    ...
    前摇: 10,
    _效果数组: [
      { 原型: '伤害结算', 目标: '单体', 生效方式: '独立生效', 威力倍率: 190, 伤害类型: '物理伤害', 防御穿透: 35 },
  ...
  5: {
    ...
    前摇: 10,
    _效果数组: [
      { 原型: '伤害结算', 目标: '单体', 生效方式: '独立生效', 威力倍率: 165, 伤害类型: '物理伤害', 防御穿透: 20 },
      {
        原型: '状态施加',
        目标: '单体',
        生效方式: '跟随主原型',
  ...
  6: {
    ...
    前摇: 10,
    _效果数组: [
      { 原型: '伤害结算', 目标: '群体', 生效方式: '独立生效', 威力倍率: 110, 伤害类型: '精神伤害' },
  ...
  10: {
    ...
    前摇: 15,
    _效果数组: [
      { 原型: '伤害结算', 目标: '单体', 生效方式: '独立生效', 威力倍率: 300, 伤害类型: '物理伤害', 防御穿透: 50 },
      { 原型: '伤害结算', 目标: '单体', 生效方式: '跟随主原型', 威力倍率: 80, 伤害类型: '真实伤害' },
    ],
  },
  11: {
    魂技名: '【11层·龙罡与龙威(被动)】',
  ...
  12: {
    ...
    前摇: 30,
    _效果数组: [
      { 原型: '伤害结算', 目标: '群体', 生效方式: '独立生效', 威力倍率: 250, 伤害类型: '物理伤害', 防御穿透: 30 },
      {
        原型: '状态施加',
        目标: '群体',
        生效方式: '跟随主原型',
...
var 银龙王血脉技能_V1 = {
  '火元素调用·火球': {
    ...
    效果描述: '调用火元素攻击单体，造成高威力能量伤害。',
    消耗: '魂力:8% | 精神力:4%',
    前摇: 8,
    附带属性: ['火'],
    _效果数组: [
      { 原型: '伤害结算', 目标: '单体', 生效方式: '独立生效', 威力倍率: 150, 伤害类型: '能量伤害' },
    ],
  },
  '水元素调用·水球': {
    魂技名: '水元素调用·水球',
    ...
    附带属性: ['水'],
    _效果数组: [
      { 原型: '伤害结算', 目标: '单体', 生效方式: '独立生效', 威力倍率: 110, 伤害类型: '能量伤害', 条件分支: [{ 条件: [{ 类型: '目标', 对象: '目标', 比较: '==', 值: '敌方' }], 处理: '生效' }] },
  ...
  '风元素调用·切割': {
    ...
    效果描述: '低前摇单体切割，命中稳定并造成多段能量伤害。',
    消耗: '魂力:6% | 精神力:4%',
    前摇: 5,
    附带属性: ['风'],
    _效果数组: [
      { 原型: '伤害结算', 目标: '单体', 生效方式: '独立生效', 威力倍率: 80, 攻击段数: 2, 伤害类型: '能量伤害' },
  ...
  '土元素调用·岩盾': {
    ...
    效果描述: '为友方生成护盾；对敌方施加短暂位移限制。',
    消耗: '魂力:7% | 精神力:4%',
    前摇: 8,
    附带属性: ['土'],
    _效果数组: [
      { 原型: '护盾变化', 目标: '单体', 生效方式: '独立生效', 护盾模式: '正向护盾', 数值: '+18%', 条件分支: [{ 条件: [{ 类型: '目标', 对象: '目标', 比较: '==', 值: '己方' }], 处理: '生效' }] },
      { 原型: '状态施加', 目标: '单体', 生效方式: '独立生效', 状态: '位移限制', 持续回合: 1, 条件分支: [{ 条件: [{ 类型: '目标', 对象: '目标', 比较: '==', 值: '敌方' }], 处理: '生效' }] },
    ],
  },
  '光元素调用·生命共享': {
    魂技名: '光元素调用·生命共享',
  ...
  '水火交融': {
    ...
    附带属性: ['水', '火'],
    _效果数组: [
      { 原型: '伤害结算', 目标: '群体', 生效方式: '独立生效', 威力倍率: 210, 伤害类型: '能量伤害' },
  ...
  '四元素风暴': {
    ...
    效果描述: '四元素群体压制，造成六环水平伤害并附带迟缓与位移限制。',
    消耗: '魂力:25% | 精神力:25%',
    前摇: 30,
    附带属性: ['水', '火', '风', '土'],
    _效果数组: [
      { 原型: '伤害结算', 目标: '群体', 生效方式: '独立生效', 威力倍率: 430, 伤害类型: '能量伤害' },
      { 原型: '判定修正', 目标: '群体', 生效方式: '跟随主原型', 判定: '反应', 数值: '-25%', 持续回合: 2 },
      { 原型: '状态施加', 目标: '群体', 生效方式: '跟随主原型', 状态: '位移限制', 持续回合: 1 },
    ],
  },
  '五元素空间裂变': {
    魂技名: '五元素空间裂变',
    ...
    附带属性: ['水', '火', '风', '土', '空间'],
    _效果数组: [
      { 原型: '伤害结算', 目标: '群体', 生效方式: '独立生效', 威力倍率: 620, 伤害类型: '能量伤害', 防御穿透: 20 },
      { 原型: '状态施加', 目标: '群体', 生效方式: '跟随主原型', 状态: '位移限制', 持续回合: 2 },
  ...
  '七元素爆裂': {
    ...
    附带属性: [...银龙王初始可调用元素_V1],
    _效果数组: [
      { 原型: '伤害结算', 目标: '群体', 生效方式: '独立生效', 威力倍率: 1000, 伤害类型: '能量伤害', 防御穿透: 30 },
      { 原型: '状态施加', 目标: '群体', 生效方式: '跟随主原型', 状态: '眩晕', 持续回合: 1 },
...
function autoGenerateSkill(
  ...
  const appendPackedEffectConditionBranches = (effect, branchList = []) => {
    ...
    const baseBranches = Array.isArray(effect.条件分支) ? clonePackedSkillEffects(effect.条件分支) : [];
      ...
      if (next.转为伤害) {
        replacement.原型 = '伤害结算';
        replacement.伤害类型 = next.伤害类型 || replacement.伤害类型 || '能量伤害';
