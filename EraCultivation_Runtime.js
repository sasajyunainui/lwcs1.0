...
!(function (global) {
  ...
  const VERSION = '1.1.0-era-cultivation-20260829';
  const PARAMETER_VERSION = 'era-cultivation-final-20260829-youth-curve-v1';
  ...
  const D3_YOUTH_INITIAL_LEVEL_ANCHORS = Object.freeze({
    劣等: Object.freeze([[6, 1.5], [9, 6.5], [18, 21.5]]),
    正常: Object.freeze([[6, 3.5], [9, 11.5], [18, 26.5]]),
    优秀: Object.freeze([[6, 5.5], [9, 13.5], [18, 38.5]]),
    天才: Object.freeze([[6, 8], [9, 21.5], [18, 49.5]]),
    顶级天才: Object.freeze([[6, 9.5], [9, 21.5], [18, 53.5]]),
    绝世妖孽: Object.freeze([[6, 10], [9, 21.5], [18, 53.5]]),
  });
  ...
  function estimateInitialLevel(options = {}) {
    ...
    if (era === '斗三' && targetAge <= 18) {
      const anchors = D3_YOUTH_INITIAL_LEVEL_ANCHORS[talent];
      const [left, right] = targetAge <= anchors[1][0] ? [anchors[0], anchors[1]] : [anchors[1], anchors[2]];
      const progress = clamp((targetAge - left[0]) / (right[0] - left[0]), 0, 1);
      const targetLevel = left[1] + (right[1] - left[1]) * progress;
      return Math.max(1, targetLevel + (variation - 1) * 10);
    }
