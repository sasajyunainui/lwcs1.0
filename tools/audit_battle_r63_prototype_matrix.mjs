...
for (const [prototype, effect] of Object.entries(effects)) {
  ...
  matrix.push({
    ...
    contributions: result.contributions.map(entry => entry.outcomeKind),
    scheduled: result.scheduledEvents.map(event => event.type),
...
assert.equal(grantResult.contributions.join(','), 'STATE_CHANGED', '机制授予没有建立可供下一次行动消费的真实状态');
const grantedActor = Object.values(grantResult.afterSnapshot?.参战者 || {})
  .flatMap(side => Array.isArray(side) ? side : Object.values(side || {}))
  .find(unit => preview.unitId(unit) === 'actor');
const pendingGrant = Object.values(grantedActor?.状态效果 || {}).find(state =>
  state?.状态 === '机制授予' &&
  /下次行动/.test(String(state?.授予触发条件 || '')) &&
  Array.isArray(state?.授予效果)
);
assert.ok(pendingGrant, '机制授予没有保留触发条件与待消费效果');
assert.ok(
  !grantResult.contributions.some(entry => ['HP_DELTA', 'SHIELD_DELTA', 'RESOURCE_DELTA'].includes(entry)),
  '机制授予在下一次行动前提前兑现了生命或资源效果',
);
