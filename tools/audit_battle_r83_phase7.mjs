...
noOpFallbackRequest.frozenCandidates = [
  ...
  {
    candidateId: 'fallback:defend',
    declaration: { actionKind: 'DEFEND', actorId: 'actor', targetIds: ['actor'] },
    costs: {},
  },
];
noOpFallbackRequest.actorCandidateRoutes = {
  ...
  'fallback:defend': {
    ...route({ routeKey: 'route:fallback-defend', targetIds: ['actor'] }),
    candidateId: 'fallback:defend',
  },
};
noOpFallbackRequest.responseModelByCandidate = {
  test: noOpFallbackRequest.responseModelByCandidate.test,
  'fallback:defend': noOpFallbackRequest.responseModelByCandidate.test,
};
...
add(
  'oracle:negative-natural-action-yields-no-op-equivalent-fallback',
  noOpFallbackResult.selected.candidateId === 'fallback:defend' &&
    noOpFallbackResult.decisionProfile.selectionMode === 'FORCED_DEFEND_FALLBACK' &&
    noOpFallbackResult.selected.forcedFallback === true &&
    noOpFallbackResult.selected.fallbackReason === 'NO_ELIGIBLE_CANDIDATE' &&
    noOpFallbackResult.selected.fallbackSourceRejectionCode ===
      'ACTIVE_DEFENSE_WITHOUT_WINDOW_VALUED' &&
...
add(
  ...
  runtimeAdaptationResult.audit?.fatalCount === 0 &&
    ...
    runtimeAdaptationDecisions[4]?.selected?.candidateId.includes('adaptation-reliable') &&
    runtimeAdaptationMisses.length === 3 &&
    runtimeAdaptationMisses.every((observation, index) =>
      index === 0 ||
      observation.posterior < runtimeAdaptationMisses[index - 1].posterior
    ) &&
