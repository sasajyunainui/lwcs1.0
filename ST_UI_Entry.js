!(function () {
  ...
  const 默认资源基础地址 = 'https://testingcf.jsdelivr.net/gh/sasajyunainui/lwcs1.0@79417c4336793e7e36b2eeafab6239529f51f038/';
  const 资源基础地址 = (() => {
  ...
  const 模块注册表 = {
    ...
    战斗运行时: { 类型: 'inline-js', 地址: 资源基础地址 + 'BattleRuntime_Module.js' + 资源版本后缀, 关键: false, 分组: 'lazy', 依赖: ['战斗预估运行时'] },
  ...
  const 热更新重置模块顺序 = Object.freeze(['内置角色库', '内置物品库', ...变量运行时接口模块顺序, '逻辑桥接', '数据库适配器', '请求监控挂件', '战斗预估运行时', '战斗运行时', '战斗模块', '数据库模块']);
  const 启动预取模块顺序 = Object.freeze(['样式核心', '魂环引擎样式', '壳层运行时', '内置角色库', '内置物品库', '逻辑桥接', '数据库适配器', '请求监控挂件', '数据库模块', '战斗预估运行时', '战斗运行时', '战斗模块']);
