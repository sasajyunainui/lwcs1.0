...
const DesktopUnifiedLayout = {
  ...
  setup() {
    ...
    const syncUnifiedFrameViewport = () => {
      ...
      const 面板矩形 = frame.getBoundingClientRect();
      const 底部边界 = 发送栏矩形 && 发送栏矩形.top > 80 && 发送栏矩形.top < 视口高度
        ? 发送栏矩形.top - 24
        : 视口高度 - 发送栏高度 - 24;
      const 顶部边界 = 面板矩形 && 面板矩形.top > 0 && 面板矩形.top < 底部边界
        ? 面板矩形.top
        : 42;
      const 可用高度 = Math.max(260, 底部边界 - 顶部边界);
      const 状态栏目标高度 = Math.min(可用高度, 视口高度 * 0.70);
      const 状态栏高度上限 = 760;
      const 状态栏高度下限 = Math.min(420, 可用高度);
      ...
      if (detailHost && detailHost.isConnected) {
        ...
        const 详情最大高度 = Math.max(260, 详情页高度 || (状态栏最大高度 - toolbarHeight - 18));
