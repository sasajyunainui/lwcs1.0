...
function 清理运行时地点候选表格行_V1(行 = {}) {
  return {
    ...
    完整路径: String(行.完整路径 || '').trim() || '无',
    父级: String(行.父级 || '').trim() || '无',
    关系: String(行.关系 || '').trim() || '无',
    步行描点: String(行.步行描点 || '').trim() || '无',
...
function 构建运行时地点候选表格行_V1(数据根 = {}, 候选 = {}) {
  ...
  (Array.isArray(候选.动态地点) ? 候选.动态地点 : []).forEach(地点名 => {
    ...
    const 叶名 = 显式父级路径 ? String(地点名 || '').trim() : 名称片段[名称片段.length - 1] || String(地点名 || '').trim();
    if (!父级路径 || !叶名) return;
    const 完整路径 = `${父级路径}-${叶名}`;
...
function 构建MVU角色卡表格文本_V1(角色表 = {}, 当前tick = null, 物品定义源 = {}) {
  ...
  return [
    ...
    渲染MVU角色卡表格_V1('武魂概览表', ['角色', '武魂槽位', '名称', '系别', '属性体系', '容纳属性', '描述'], 表.武魂概览),
