...
function 收集远端原著时间线实体锚点_V1(数据根 = {}, 捕获文本 = '', 运行补充文本 = '') {
  ...
  合并文本命中(捕获文本, 1);
  合并文本命中(运行补充文本, 0.45);
  添加(地点, [当前地点, 当前上下文节点, 当前路径, 当前路径.length ? 构建运行时地点路径名_V1(当前路径) : ''], 0.7);
...
function 计算远端原著时间线候选分数_V1(事件 = {}, 上下文 = {}) {
  ...
  const 有组织同场命中 = !!(组织场景锚点命中.length || 地点命中.length || 势力命中.length || 动态地点命中.length);
  ...
  if (具体场景锚点命中.length && 事件词命中.length && (!班级场景锚点命中.length || 有组织同场命中)) {
    追加组合命中(
      具体场景锚点命中[0],
      事件词命中[0],
  ...
  if (具体场景锚点命中.length && 非主角人物命中.length && (!班级场景锚点命中.length || 有组织同场命中)) {
    追加组合命中(具体场景锚点命中[0], 非主角人物命中[0], 2.5);
  }
  if (组织场景锚点命中.length && (地点命中.length || 势力命中.length || 动态地点命中.length)) {
    追加组合命中(组织场景锚点命中[0], 地点命中[0] || 势力命中[0] || 动态地点命中[0], 1.8);
  } else if (班级场景锚点命中.length && 有组织同场命中) {
    追加组合命中(班级场景锚点命中[0], 组织场景锚点命中[0] || 地点命中[0] || 势力命中[0] || 动态地点命中[0], 2.8);
  }
