...
function 生成MVU更新结构提示_V1(数据输入 = null, userInput = '', 最后角色消息输入 = '', plotText = '') {
  ...
  '1. 【重要实体差分】: Compare characters/places in this reply with "Existing MVU Entity Hits". First check if the entity already exists in the hits; if it exists, you are strictly PROHIBITED from adding it again! Only register when a completely NEW character/place with a proper name enters the long-term plot.',
  '2. 【过滤临时过场】: Strictly IGNORE generic descriptive NPCs (e.g., "板寸头", "瘦高个", "胖宿管") and one-off background places. Do NOT create MVU entities for them.',
  '',
  'New Entity Table:',
  'char=填入具备正式姓名的持久新角色(若无填无); world.动态地点=填入新重要地点(若无填无); org=填入新势力(若无填无); 物品=填入新物品(若无填无).',
    '',
...
function 收集远端原著时间线实体锚点_V1(数据根 = {}, 查询文本 = '') {
  const 当前tick = Number(数据根?.world?.时间?.tick || 0);
  const 命中名称 = 收集运行时命中名称_V1(数据根, 查询文本);
  const 角色 = new Set([
    ...Array.from(命中名称.角色 || []),
    ...匹配文本内置角色名_V1(查询文本, 当前tick, 数据根),
    ...收集运行时字符串列表_V1(数据根?.相关实体索引?.角色),
  ]);
  const 实体 = new Set([
    ...Array.from(命中名称.地点 || []),
    ...Array.from(命中名称.动态地点 || []),
    ...Array.from(命中名称.势力 || []),
    ...Array.from(命中名称.物品 || []),
    ...收集运行时字符串列表_V1(
      数据根?.相关实体索引?.命中地点,
...
function 计算远端原著时间线候选分数_V1(事件 = {}, 上下文 = {}) {
  ...
  if (非主角人物命中.length) {
    分数 += 14 + Math.min(10, 非主角人物命中.length * 4);
    有硬锚点 = true;
    原因.push(`人物:${非主角人物命中.slice(0, 3).join('/')}`);
  }
  const 泛主角命中 = 人物命中.filter(名称 => 远端原著时间线泛主角名_V1.has(名称));
  if (泛主角命中.length) {
    分数 += 非主角人物命中.length ? 4 : 2;
    有硬锚点 = 有硬锚点 || 非主角人物命中.length > 0;
    原因.push(`泛主角:${泛主角命中.slice(0, 2).join('/')}`);
  }
  const 实体命中 = Array.from(上下文.实体 || []).filter(名称 => 运行时文本包含片段_V1(事件文本, 名称));
  if (实体命中.length) {
    分数 += 8 + Math.min(12, 实体命中.length * 3);
    有硬锚点 = true;
    原因.push(`实体:${实体命中.slice(0, 3).join('/')}`);
  }
  const 关键词命中 = (上下文.关键词 || []).filter(关键词 => 运行时文本包含片段_V1(事件文本, 关键词));
  if (关键词命中.length) {
    分数 += Math.min(12, 关键词命中.length * 2);
    原因.push(`关键词:${关键词命中.slice(0, 4).join('/')}`);
  }
  const 事件词命中 = 远端原著时间线事件词_V1.filter(关键词 => (
    运行时文本包含片段_V1(上下文.查询文本, 关键词) && 运行时文本包含片段_V1(事件文本, 关键词)
  ));
  if (事件词命中.length) {
    分数 += Math.min(12, 事件词命中.length * 3);
    原因.push(`事件词:${事件词命中.slice(0, 3).join('/')}`);
  }
  if (!有硬锚点 && 分数 < 8) return null;
  if (!有硬锚点 && 泛主角命中.length && !实体命中.length && 关键词命中.length < 3) return null;
  return {
...
function 收集远端原著时间线候选_V1(数据根 = {}, 用户输入 = '', 最大数量 = 20) {
  ...
  return 收集原著时间线事件列表_V1()
    ...
    .filter(Boolean)
    .sort((左, 右) => 右.分数 - 左.分数 || 左.触发tick - 右.触发tick || 左.标识.localeCompare(右.标识, 'zh-Hans-CN', { numeric: true, sensitivity: 'base' }))
    .slice(0, Math.max(1, Number(最大数量 || 20)));
}
...
function 构建远端原著时间线候选文本_V1(数据根 = {}, 用户输入 = '', 最大数量 = 20) {
  ...
  return 候选列表.map(事件 => {
    const 人物 = 事件.人物.length ? 事件.人物.join('、') : '无';
    return `${事件.标识}｜tick ${事件.触发tick}｜人物：${人物}｜原著事件：${事件.原著事件}｜命中原因：${事件.命中原因}`;
  }).join('\n');
}
...
function 格式化MVU剧情提示值_V1(值, 选项 = {}) {
  if (Array.isArray(值)) {
    return 值
      .map(项 => (项 && typeof 项 === 'object' ? 格式化MVU剧情提示对象片段_V1(项, 4) : 格式化MVU剧情提示原子值_V1(项, 选项)))
      .filter(Boolean)
      .join('、');
  }
  if (值 && typeof 值 === 'object') return 格式化MVU剧情提示对象片段_V1(值, 5);
...
function 构建MVU剧情提示当前位置行动_V1(角色 = {}) {
  const 片段列表 = [];
  追加MVU剧情提示片段_V1(片段列表, '位置', 角色?.状态?.位置);
  追加MVU剧情提示片段_V1(片段列表, '行动', 角色?.状态?.行动);
  return 片段列表.join('；');
}
...
function 构建MVU剧情提示外貌穿搭_V1(角色 = {}) {
  ...
  追加MVU剧情提示片段_V1(片段列表, '特征', 角色?.外貌?.特殊特征);
  追加MVU剧情提示片段_V1(片段列表, '穿搭', 角色?.穿搭?.描述);
  return 片段列表.join('；');
}
...
function 构建MVU剧情提示当前段_V1(数据根 = {}, userInput = '') {
  const { 玩家名, 当前地点 } = 取运行时当前范围_V1(数据根);
  const 片段列表 = [];
  追加MVU剧情提示片段_V1(片段列表, '时间', 格式化MVU剧情提示当前时间_V1(数据根?.world?.时间 || {}));
  追加MVU剧情提示片段_V1(片段列表, '地点', 当前地点);
  追加MVU剧情提示片段_V1(片段列表, '玩家', 玩家名);
  追加MVU剧情提示片段_V1(片段列表, '玩家行动', userInput);
  追加MVU剧情提示片段_V1(片段列表, '系统播报', 数据根?.sys?.系统播报);
...
function 构建MVU剧情提示引导段_V1(数据根 = {}, userInput = '') {
  const 文本 = String(userInput || '');
  const 当前tick = Number(数据根?.world?.时间?.tick || 0);
  const 时间线摘要 = 构建运行时未来事件视图_V1(数据根?.world?.时间线 || {}, 4, { 派生时间文本: true, 当前tick });
  const 委托摘要 = 复制运行时命中记录表片段_V1(数据根?.world?.委托板 || {}, 文本, 3, 构建运行时委托草案条目_V1);
  const 拍卖摘要 = 构建运行时拍卖薄片_V1(数据根?.world?.拍卖 || {}, 文本, 3);
  const 战斗摘要 = 构建MVU战斗摘要_V1(数据根?.world?.战斗);
  const 行列表 = ['【剧情引导】'];
  [
    ['时间线', 时间线摘要],
...
function 生成MVU剧情提示文本_V1(数据输入 = null, userInput = '', 最后剧情文本 = '') {
  const 数据根 = 读取运行时Mvu数据根或最新_V1(数据输入) || {};
  return [
    构建MVU剧情提示当前段_V1(数据根, userInput),
    构建MVU剧情提示角色段_V1(数据根, userInput, 最后剧情文本),
    构建MVU剧情提示引导段_V1(数据根, userInput),
  ].filter(Boolean).join('\n\n');
}
...
function 构建MVU正文角色卡_V1(角色名 = '', 角色 = {}, 正文角色表 = {}) {
  ...
  const 已展示字段 = new Set(['状态', '外貌', '穿搭', '性格', '财富', '社交', '副职业', '武魂融合技', '装备', '背包', '我的任务', '任务', '属性']);
...
function 生成角色基础六维对标摘要_V1(数据输入 = null, userInput = '') {
  ...
  角色名列表.forEach(角色名 => {
    ...
    const 副职业文本 = Object.entries(角色?.副职业 || {})
      ...
      .filter(Boolean)
      .join('，');
    行列表.push(`${角色名} ${等级文本}：${字段文本}${副职业文本 ? `；副职业：${副职业文本}` : ''}`);
  });
...
function 替换MVU运行时视图占位符_V1(文本 = '', 视图类型 = 'empty', 上下文 = {}) {
  const 源文本 = String(文本 || '');
  if (
    !源文本.includes(MVU_RUNTIME_VIEW_PLACEHOLDER_V1) &&
    !源文本.includes(MVU_RUNTIME_UPDATE_PLACEHOLDER_V1) &&
    !源文本.includes(MVU_UPDATE_STRUCTURE_HINTS_PLACEHOLDER_V1)
  ) return 源文本;
  ...
  const 更新视图 = 生成MVU更新视图_V1(数据根, userInput, 最后角色消息输入, plotText);
  const 剧情视图 = 生成MVU剧情视图_V1(数据根, userInput);
  ...
  const 主视图 = 视图类型文本 === 'empty' ? {} : (视图类型文本 === 'update' ? 更新视图 : (视图类型文本 === 'plot' ? 剧情视图 : 正文视图));
  const 主视图文本 = 视图类型文本 === 'plot'
    ? 剧情提示文本
    : (视图类型文本 === 'story' ? 正文提示文本 : 序列化MVU运行时视图_V1(主视图));
  const 结构提示 = 生成MVU更新结构提示_V1(数据根, userInput, 最后角色消息输入, plotText);
  const 替换后 = 源文本
    .replaceAll(MVU_RUNTIME_VIEW_PLACEHOLDER_V1, 主视图文本)
    .replaceAll(MVU_RUNTIME_UPDATE_PLACEHOLDER_V1, 序列化MVU运行时视图_V1(更新视图))
