  ...
  Object.entries(角色表 || {}).forEach(([角色名, 角色]) => {
    ...
    取角色武魂条目_V1(角色).forEach(([武魂键, 武魂]) => {
      ...
      取武魂魂灵条目_V1(武魂).forEach(([魂灵键, 魂灵]) => {
        ...
        添加MVU角色卡表格行_V1(表.武魂, { 角色: 角色名, 武魂: 武魂显示名, 魂灵: [魂灵键, 魂灵文本].filter(Boolean).join(' '), 魂环: '', 魂技: '', 效果: '', 画面: '', 产物: '', 备注: 魂灵?.战力面板?.对标等级 }, 覆盖器, []);
        取魂灵魂环条目_V1(魂灵).forEach(([魂环键, 魂环]) => {
          取魂环魂技条目_V1(魂环).forEach(([魂技键, 魂技]) => {
      ...
      取武魂直接魂环条目_V1(武魂).forEach(([魂环键, 魂环]) => {
        ...
        if (!魂技条目.length) 添加MVU角色卡表格行_V1(表.武魂, { 角色: 角色名, 武魂: 武魂显示名, 魂灵: '', 魂环: [魂环键, 魂环?.年限 ? `${魂环.年限}年` : '', 魂环?.颜色].filter(Boolean).join(' '), 魂技: '', 效果: '', 画面: '', 产物: '', 备注: '' }, 覆盖器, []);
        魂技条目.forEach(([魂技键, 魂技]) => {
