...
function 构建MVU正文角色属性薄片_V1(角色 = {}) {
  ...
  const 输出 = {};
  ['年龄', '生日', '性别', '等级', '天赋梯队', '邪魂师', '精神境界', '力量', '防御', '敏捷'].forEach(字段 => {
...
function 渲染MVU角色卡表格_V1(标题 = '', 表头 = [], 行列表 = []) {
  ...
  const 头 = 表头.map(列 => String(列 || '').trim()).filter(Boolean);
  if (!头.length) return '';
  const 表格行 = [
    `【${标题}】`,
    `| ${头.join(' | ')} |`,
...
function 构建MVU角色卡表格数据_V1(角色表 = {}, 当前tick = null) {
  ...
  Object.entries(角色表 || {}).forEach(([角色名, 角色]) => {
    ...
    }, 覆盖器, ['属性.性别', '属性.年龄', '属性.生日', '属性.等级', '属性.天赋梯队', '社交.主身份', '社交.家世描述', '外貌', '穿搭', '性格', '社交.名望等级', '社交.声望']);
    const 状态摘要 = 构建MVU正文状态摘要_V1(角色);
    添加MVU角色卡表格行_V1(表.现场, {
      角色: 角色名,
      当前地点: 构建MVU角色卡地点描述_V1(角色),
    ...
    const 对标 = 角色.基础六维对标 && typeof 角色.基础六维对标 === 'object'
      ? 角色.基础六维对标
      : 构建角色基础六维对标条目_V1(角色, 当前tick);
    添加MVU角色卡表格行_V1(表.实力, {
      角色: 角色名,
      年龄: 对标.年龄,
      等级: 对标.等级,
      ...
      '体力/气血': 对标['体力/气血'],
      精神力: 对标.精神力,
      备注: 对标.非魂师 ? '非魂师' : 对标.副职业,
    }, 覆盖器, ['基础六维对标', '属性.力量', '属性.防御', '属性.敏捷', '属性.体力上限', '属性.精神力上限']);
    取角色武魂条目_V1(角色).forEach(([武魂键, 武魂]) => {
      const 武魂名 = 格式化MVU正文提示原子值_V1(武魂?.表象名称 || 武魂?.名称 || '');
      添加MVU角色卡表格行_V1(表.武魂, {
        角色: 角色名,
        武魂: 取MVU正文武魂显示名_V1(武魂键),
        魂灵: '',
        魂环: '',
        魂技: 武魂名,
        效果: 武魂?.描述,
      ...
      取武魂魂灵条目_V1(武魂).forEach(([魂灵键, 魂灵]) => {
        ...
        添加MVU角色卡表格行_V1(表.武魂, { 角色: 角色名, 武魂: 取MVU正文武魂显示名_V1(武魂键), 魂灵: 魂灵键, 魂环: '', 魂技: 魂灵文本, 效果: '', 画面: '', 产物: '', 备注: 魂灵?.战力面板?.对标等级 }, 覆盖器, []);
        取魂灵魂环条目_V1(魂灵).forEach(([魂环键, 魂环]) => {
          取魂环魂技条目_V1(魂环).forEach(([魂技键, 魂技]) => {
            添加MVU角色卡表格行_V1(表.武魂, { 角色: 角色名, 武魂: 取MVU正文武魂显示名_V1(武魂键), 魂灵: 魂灵键, 魂环: [魂环键, 魂环?.年限 ? `${魂环.年限}年` : '', 魂环?.颜色].filter(Boolean).join(' '), 魂技: 魂技?.魂技名 || 魂技?.name || 魂技键, 效果: 魂技?.效果描述 || 魂技?.描述, 画面: 魂技?.画面描述, 产物: 魂技?.产物描述, 备注: '' }, 覆盖器, []);
          });
        });
      });
      取武魂直接魂环条目_V1(武魂).forEach(([魂环键, 魂环]) => {
        const 魂技条目 = 取魂环魂技条目_V1(魂环);
        if (!魂技条目.length) 添加MVU角色卡表格行_V1(表.武魂, { 角色: 角色名, 武魂: 取MVU正文武魂显示名_V1(武魂键), 魂灵: '', 魂环: [魂环键, 魂环?.年限 ? `${魂环.年限}年` : '', 魂环?.颜色].filter(Boolean).join(' '), 魂技: '', 效果: '', 画面: '', 产物: '', 备注: '' }, 覆盖器, []);
        魂技条目.forEach(([魂技键, 魂技]) => {
          添加MVU角色卡表格行_V1(表.武魂, { 角色: 角色名, 武魂: 取MVU正文武魂显示名_V1(武魂键), 魂灵: '', 魂环: [魂环键, 魂环?.年限 ? `${魂环.年限}年` : '', 魂环?.颜色].filter(Boolean).join(' '), 魂技: 魂技?.魂技名 || 魂技?.name || 魂技键, 效果: 魂技?.效果描述 || 魂技?.描述, 画面: 魂技?.画面描述, 产物: 魂技?.产物描述, 备注: '' }, 覆盖器, []);
        });
      });
      覆盖器.markSubtree(武魂键);
    });
    Object.entries(构建MVU正文副职业摘要_V1(角色)).forEach(([项目, 内容]) => 添加MVU角色卡表格行_V1(表.成长, { 角色: 角色名, 类型: '副职业', 项目, 内容 }, 覆盖器, []));
    ['功法', '自创魂技', '精神领域', '血脉之力', '魂核', '复制效果'].forEach(字段 => {
      const 值 = 角色?.[字段];
      if (!值 || typeof 值 !== 'object') return;
      Object.entries(值).forEach(([项目, 内容]) => 添加MVU角色卡表格行_V1(表.成长, { 角色: 角色名, 类型: 字段, 项目, 内容 }, 覆盖器, []));
      覆盖器.markSubtree(字段);
    });
    Object.entries(构建MVU正文武魂融合技摘要_V1(角色名, 角色, 角色表)).forEach(([项目, 内容]) => 添加MVU角色卡表格行_V1(表.成长, { 角色: 角色名, 类型: '武魂融合技', 项目, 内容 }, 覆盖器, []));
    ...
    Object.entries(社交.势力 || {}).forEach(([势力, 数据]) => 添加MVU角色卡表格行_V1(表.势力称号, { 角色: 角色名, 类型: '势力', 名称: 势力, 身份: 数据?.身份, 权限: 数据?.权限级, 来源: '', 加成: '' }, 覆盖器, []));
    Object.entries(社交.称号 || {}).forEach(([称号, 数据]) => 添加MVU角色卡表格行_V1(表.势力称号, { 角色: 角色名, 类型: '称号', 名称: 称号, 身份: '', 权限: '', 来源: 数据?.来源, 加成: 数据?.声望加成 }, 覆盖器, []));
    覆盖器.markSubtree('社交.势力');
    覆盖器.markSubtree('社交.称号');
    Object.entries(构建MVU正文装备摘要_V1(角色?.装备 || {})).forEach(([槽位, 内容]) => 添加MVU角色卡表格行_V1(表.装备物品, { 角色: 角色名, 类型: '装备', 名称: 槽位, 数量: '', 状态: 内容?.装备状态, 内容 }, 覆盖器, []));
    Object.entries(角色?.魂骨 || {}).forEach(([名称, 内容]) => 添加MVU角色卡表格行_V1(表.装备物品, { 角色: 角色名, 类型: '魂骨', 名称, 数量: '', 状态: 内容?.装备状态 || 内容?.状态, 内容 }, 覆盖器, []));
    Object.entries(角色?.背包 || {}).forEach(([名称, 内容]) => 添加MVU角色卡表格行_V1(表.装备物品, { 角色: 角色名, 类型: '背包', 名称, 数量: 内容?.数量, 状态: '', 内容 }, 覆盖器, []));
    Object.entries(构建MVU正文财富摘要_V1(角色)).forEach(([名称, 内容]) => 添加MVU角色卡表格行_V1(表.装备物品, { 角色: 角色名, 类型: '财富', 名称, 数量: 内容, 状态: '', 内容: '' }, 覆盖器, []));
    ['装备', '魂骨', '背包', '财富'].forEach(字段 => 覆盖器.markSubtree(字段));
    Object.entries(角色?.我的任务 || {}).forEach(([任务, 内容]) => 添加MVU角色卡表格行_V1(表.剧情, { 角色: 角色名, 类型: '我的任务', 名称: 任务, 状态: 内容?.状态, 内容 }, 覆盖器, []));
    Object.entries(角色?.任务 || {}).forEach(([任务, 内容]) => 添加MVU角色卡表格行_V1(表.剧情, { 角色: 角色名, 类型: '任务', 名称: 任务, 状态: 内容?.状态, 内容 }, 覆盖器, []));
    (Array.isArray(角色?.已掌握情报) ? 角色.已掌握情报 : []).forEach((内容, 索引) => 添加MVU角色卡表格行_V1(表.剧情, { 角色: 角色名, 类型: '已掌握情报', 名称: `情报${索引 + 1}`, 状态: '', 内容 }, 覆盖器, []));
    Object.entries(角色?.魂灵塔记录 || {}).forEach(([名称, 内容]) => 添加MVU角色卡表格行_V1(表.剧情, { 角色: 角色名, 类型: '魂灵塔记录', 名称, 状态: '', 内容 }, 覆盖器, []));
    Object.entries(角色?.战斗历史 || {}).forEach(([名称, 内容]) => 添加MVU角色卡表格行_V1(表.剧情, { 角色: 角色名, 类型: '战斗历史', 名称, 状态: 内容?.最近结果, 内容 }, 覆盖器, []));
    ['我的任务', '任务', '已掌握情报', '魂灵塔记录', '战斗历史'].forEach(字段 => 覆盖器.markSubtree(字段));
    收集MVU角色卡有效叶子路径_V1(角色).forEach(({ 路径, 值 }) => {
      if (!路径 || 覆盖器.isExcluded(路径) || 覆盖器.isCovered(路径)) return;
      添加MVU角色卡表格行_V1(表.扩展, { 角色: 角色名, 字段路径: 路径, 内容: 值 }, 覆盖器, [路径]);
    });
  });
  const 有效字段数 = 覆盖器.覆盖路径.size + 覆盖器.覆盖子树.size;
  const 排除字段数 = 覆盖器.排除路径.size;
  const 扩展字段数 = 表.扩展.length;
  return { 表, 覆盖率: { 有效字段数, 排除字段数, 扩展字段数, 遗漏字段数: 0 } };
}

function 构建MVU角色卡表格文本_V1(角色表 = {}, 当前tick = null) {
  const { 表 } = 构建MVU角色卡表格数据_V1(角色表, 当前tick);
  return [
    渲染MVU角色卡表格_V1('角色基础表', ['角色', '性别', '年龄', '生日', '等级', '天赋', '主身份', '家世', '外貌', '穿搭', '性格', '名望'], 表.基础),
    渲染MVU角色卡表格_V1('角色现场表', ['角色', '当前地点', '生死', '死亡类型', '伤势', '当前状态', '剩余资源'], 表.现场),
    渲染MVU角色卡表格_V1('实力对标表', ['角色', '年龄', '等级', '力量', '防御', '敏捷', '体力/气血', '精神力', '备注'], 表.实力),
    渲染MVU角色卡表格_V1('武魂魂技表', ['角色', '武魂', '魂灵', '魂环', '魂技', '效果', '画面', '产物', '备注'], 表.武魂),
    渲染MVU角色卡表格_V1('成长能力表', ['角色', '类型', '项目', '内容'], 表.成长),
    渲染MVU角色卡表格_V1('社交关系表', ['角色', '对象', '关系', '好感度', '路线', '阶段', '推进提示'], 表.关系),
    渲染MVU角色卡表格_V1('势力称号表', ['角色', '类型', '名称', '身份', '权限', '来源', '加成'], 表.势力称号),
    渲染MVU角色卡表格_V1('装备物品财富表', ['角色', '类型', '名称', '数量', '状态', '内容'], 表.装备物品),
    渲染MVU角色卡表格_V1('剧情记录表', ['角色', '类型', '名称', '状态', '内容'], 表.剧情),
    渲染MVU角色卡表格_V1('角色扩展表', ['角色', '字段路径', '内容'], 表.扩展),
  ].filter(Boolean).join('\n\n') || '无';
}
...
function 生成MVU正文提示文本_V1(数据输入 = null, userInput = '', plotText = '', 已生成正文视图 = null) {
  ...
  const 当前tick = 正文视图.world?.时间?.tick ?? null;
  const 角色卡文本 = 构建MVU角色卡表格文本_V1(正文视图.char || {}, 当前tick);
  return [
    '【角色卡】',
    角色卡文本,
