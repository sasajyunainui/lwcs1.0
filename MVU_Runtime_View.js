...
function 生成角色基础六维对标摘要_V1(数据输入 = null, userInput = '') {
  ...
  if (年龄对标行列表.length) {
    行列表.push('档位说明：* **普通**：【大路货】大众水平,NPC毫无波澜。\n* **优秀**：【十里挑一】普通班尖子生/重点班中游，一个百人年级也就十来个。表现亮眼的好苗子，NPC的反应是“挺不错、底子扎实”\n* **天才**：【百里挑一】学校一届也就2-3个。地方尖子生，NPC表现为“赞许或正常重视”\n* **顶级天才**：【万里挑一】大陆一年仅几个。顶尖势力会真正侧目与投资。\n* **绝世妖孽**：【时代孤品】时代断层，几十年仅出一个。打破大陆常理的异数。');
    行列表.push(...年龄对标行列表);
  }
  角色名列表.forEach(角色名 => {
    const 角色 = 数据根?.char?.[角色名];
