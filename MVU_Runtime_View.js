...
function 构建MVU角色卡表格数据_V1(角色表 = {}, 当前tick = null) {
  ...
  Object.entries(角色表 || {}).forEach(([角色名, 角色]) => {
    ...
    ['外貌', '穿搭', '性格'].forEach(字段 => 覆盖器.markSubtree(字段));
    ...
    if (血脉之力) {
      const 血脉显示名 = ['血脉之力', 血脉之力.血脉 || 血脉之力.名称].filter(Boolean).join('：');
      添加MVU角色卡表格行_V1(表.武魂概览, { 角色: 角色名, 类型: '血脉之力', 武魂: 血脉显示名, 描述: 血脉之力.描述 || 血脉之力.效果描述, 备注: 血脉之力.状态 || 血脉之力.核心 }, 覆盖器, []);
      ['技能', '被动'].forEach(分组 => {
    ...
    const 魂核 = 角色?.魂核 && typeof 角色.魂核 === 'object' ? 角色.魂核 : null;
    if (魂核) {
      Object.entries(魂核).forEach(([名称, 内容]) => 添加MVU角色卡表格行_V1(表.武魂概览, { 角色: 角色名, 类型: '魂核', 武魂: 名称, 描述: 内容?.描述 || 内容?.效果描述 || 内容, 备注: 内容?.状态 || 内容?.层级 }, 覆盖器, []));
      覆盖器.markSubtree('魂核');
    }
    Object.entries(构建MVU正文副职业摘要_V1(角色)).forEach(([项目, 内容]) => 添加MVU角色卡表格行_V1(表.成长, { 角色: 角色名, 类型: '副职业', 项目, 内容 }, 覆盖器, []));
    ['功法', '自创魂技', '精神领域', '复制效果'].forEach(字段 => {
      const 值 = 角色?.[字段];
...
function 构建MVU角色卡表格文本_V1(角色表 = {}, 当前tick = null) {
  ...
  return [
    渲染MVU角色卡表格_V1('角色基础表', ['角色', '性别', '年龄', '生日', '等级', '精神境界', '主身份', '家世', '外貌', '穿搭', '性格', '名望', '邪魂师'], 表.基础),
    渲染MVU角色卡表格_V1('角色现场表', ['角色', '当前地点', '生死', '死亡类型', '伤势', '当前状态', '剩余资源'], 表.现场),
    渲染MVU角色卡表格_V1('武魂概览表', ['角色', '类型', '武魂', '系别', '描述', '备注'], 表.武魂概览),
    渲染MVU角色卡表格_V1('魂技明细表', ['角色', '武魂', '魂灵', '魂环', '魂技', '效果', '画面', '产物', '备注'], 表.武魂),
