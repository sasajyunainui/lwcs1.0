...
function 计算场景背景角色人物邻近分_V1(角色名 = '', 命中 = {}) {
  ...
  const 事件文本 = 构建远端原著时间线事件检索文本_V1(命中?.事件 || {});
  if (!名称 || !事件文本 || !事件文本.includes(名称)) return 0;
  let 最高分 = 0;
  let 位置 = 事件文本.indexOf(名称);
  while (位置 >= 0) {
    const 片段 = 事件文本.slice(Math.max(0, 位置 - 56), Math.min(事件文本.length, 位置 + 名称.length + 76));
    ...
    if (/五班|零班/.test(片段) && !/一班|新生班|同届|同年级/.test(片段)) 分数 -= 5;
    最高分 = Math.max(最高分, 分数);
    位置 = 事件文本.indexOf(名称, 位置 + 名称.length);
  }
...
function 生成场景背景角色补充_V1(数据输入 = null, userInput = '', 最后剧情文本 = '', 场景线索种子 = '', 最大数量 = 8) {
  ...
  const 来源片段 = Array.from(new Set([
    ...锚点.组织锚点.slice(0, 2).map(词 => `组织锚点=${词}`),
    ...锚点.场景锚点.slice(0, 4).map(词 => `场景锚点=${词}`),
    ...锚点.事件词.slice(0, 3).map(词 => `事件词=${词}`),
  ])).join('；') || '当前场景锚点';
  const 事件片段 = Array.from(new Set(事件命中列表
    .slice(0, 3)
    .flatMap(命中 => 命中.命中生态词 || [])))
    .slice(0, 6);
  return [
    `同域候选人物：${候选人物.join('、')}`,
    `候选来源：${[来源片段, 事件片段.length ? `同域事件=${事件片段.join('/')}` : ''].filter(Boolean).join('；')}`,
    '使用口径：仅供任务2核对场景背景，不代表当前同屏、已正式分班或必须出场。',
  ].join('\n');
}
...
function 替换MVU运行时视图占位符_V1(文本 = '', 视图类型 = 'empty', 上下文 = {}) {
  ...
  if (!需要主视图 && !需要更新视图 && !需要结构提示 && !需要相互可见性 && !需要场景候选角色资料 && !需要场景背景角色补充) return 源文本;
  ...
  const 场景背景角色补充文本 = (需要场景背景角色补充 || 需要场景候选角色资料)
